...
struct Implicant {
  ...
  int ones;
  bool used;
  Implicant(int i = 0, vector < int > min = vector < int > (), string t = "", int m = 0, bool u = false): implicant(i), mask(m), ones(0), used(u) {
    if (t == "") {
      stringstream ss;
      ss << 'm' << i;
      minterms = ss.str();
...
ostream & operator << (ostream & out, const Implicant & im) 
{
  ...
  out << ss.str();
  if (pr) out << '\t' << setw(16) << left << im.minterms << ' ' << im.bits << '\t' <<
    im.ones;
  return out;
}
